...
func TestErrorUponMaximumRequeues(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			withRequeues: config.DefaultMaxReconciliationRequeues,
			expectError:  nil,
		},
		{
			name:         "Should err upon maximum reconciliation requeues exceeded for a single reconciler",
			withRequeues: config.DefaultMaxReconciliationRequeues + 1,
...
func (m mockReconciler) Reconcile(_ Context) (Result, error) {
	panic("implement me")
}
